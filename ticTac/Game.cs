...
namespace ticTac
{
	class Game
	{
        ...
        public Player Play()
        {
            ...
            while (!win && Board.RemainingBlocks > 0)
            {
                Console.Clear();
                Board.DisplayBoard();
                if (NextPlayer().TurnError)
                {
					NextPlayer().TurnError = false;
					Console.WriteLine("Typo or Position Taken.");
                } else
                {
					SwitchPlayer();
                }
				//Console.WriteLine($"{NextPlayer().Name} Turn as {NextPlayer().Marker}");
